...
class MemoryCalculator:
    ...
    def get_output_channel_memory_contribution(self, node, mask, oc_idx):
        pass


    def get_pruned_graph_memory(self, masks: Dict[BaseNode, np.ndarray], fw_impl):
        total_memory = 0.
        ...
        for n in shared_nodes:
            ...
            total_memory -= self.fw_impl.get_pruned_node_num_params(n,
                                                                    node_input_mask,
                                                                    node_output_mask,
                                                                    self.fw_info)*4.
    ...
    #
    #                 assert w.shape[ic_axis] == len(input_mask), f"Kernel num of input channels: {w.shape[ic_axis]}, but mask len is {len(input_mask)}"
    #                 assert w.shape[oc_axis] == len(output_mask), f"Kernel num of output channels: {w.shape[oc_axis]}, but mask len is {len(output_mask)}"
    #
    ...
    def _get_pruning_section_memory(self,
        ...
        """
        first_node_memory = self.fw_impl.get_pruned_node_num_params(node=pruning_section.input_node,
        ...
        for inter_node in pruning_section.intermediate_nodes:
            total_inter_nodes_memory += self.fw_impl.get_pruned_node_num_params(node=inter_node,
                                                                                input_mask=pruning_section_mask.input_node_oc_mask,
                                                                                output_mask=pruning_section_mask.input_node_oc_mask,
                                                                                fw_info=self.fw_info)

        second_node_memory = self.fw_impl.get_pruned_node_num_params(node=pruning_section.output_node,
                                                                     input_mask=pruning_section_mask.input_node_oc_mask,
