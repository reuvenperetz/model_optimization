...
# from hardware_modeling.hardware_definition.quantization.quantization_method import QuantizationMethod
...
class OpQuantizationConfig:
    def __init__(self,
                 ...
                 weights_multiplier_nbits: int = None  # If None - set 8 in hptq, ow use it
                 ):
        """

        Args:
            activation_quantization_method: Which method to use from QuantizationMethod for activation quantization.
            weights_quantization_method: Which method to use from QuantizationMethod for weights quantization.
            activation_n_bits: Number of bits to quantize the activations.
            weights_n_bits: Number of bits to quantize the coefficients.
            weights_per_channel_threshold: Whether to quantize the weights per-channel or not (per-tensor).
            enable_weights_quantization: Whether to quantize the model weights or not.
            enable_activation_quantization: Whether to quantize the model activations or not.
        """
...
class QuantizationConfigOptions(object):
    """
    Wrap a set of quantization configurations to use them for some operation sets that can
    be configured in one of the options (e.g. for mixed-precision quantization).
    """
