...
if importlib.util.find_spec("tensorflow") is not None\
    ...
    def keras_post_training_quantization(in_model: Model,
                                         ...
                                         fw_hw_model: FrameworkHardwareModel = DEFAULT_KERAS_IMX500_MODEL):
    ...
    def keras_post_training_quantization_mixed_precision(in_model: Model,
                                                         representative_data_gen: Callable,
                                                         n_iter: int = 500,
                                                         quant_config: MixedPrecisionOptimizationParams = DEFAULT_MIXEDPRECISION_CONFIG,
                                                         fw_info: FrameworkInfo = DEFAULT_KERAS_INFO,
                                                         network_editor: List[EditRule] = [],
                                                         gptq_config: GradientPTQConfig = None,
                                                         analyze_similarity: bool = False,
         ...
         **For now, mixed precision is supported for weights only.**
         ...
         Args:
             ...
             representative_data_gen (Callable): Dataset used for calibration.
             n_iter (int): Number of calibration iterations to run.
             quant_config (MixedPrecisionOptimizationParams): QuantizationConfig containing parameters of how the
         ...
         Examples:
             ...
             >>> config = mct.MixedPrecisionOptimizationParams(weights_n_bits=[4, 2, 8])
        ...
        if target_kpi is None:
            common.Logger.critical("No KPI was passed. Can not use mixed-precision compression process...")
